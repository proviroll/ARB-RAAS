...
l1chainid=1337
simple=true
parent_rpc=http://geth:8545
child_rpc=http://sequencer:8547
...
if $force_init; then
    ...
    if $tokenbridge; then
        ...
        docker compose run -e ROLLUP_OWNER_KEY=$l2ownerKey -e ROLLUP_ADDRESS=$rollupAddress -e PARENT_KEY=$devprivkey -e PARENT_RPC= $parent_rpc -e CHILD_KEY=$devprivkey -e CHILD_RPC=$child_rpc tokenbridge deploy:local:token-bridge
    ...
    docker compose run -e CHILD_CHAIN_RPC=$child_rpc -e CHAIN_OWNER_PRIVKEY=$l2ownerKey rollupcreator deploy-cachemanager-testnode
...
# echo "CHILD_RPC: $child_rpc (also used for: CHILD_CHAIN_RPC)"
